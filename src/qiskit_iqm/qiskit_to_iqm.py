...
def serialize_qubit_mapping(qubit_mapping: dict[Qubit, str], circuit: QiskitQuantumCircuit):
    return [SingleQubitMapping(logical_name=qubit_to_name(k, circuit), physical_name=v) for k, v in qubit_mapping.items()]
...
def serialize_circuit(circuit: QiskitQuantumCircuit) -> Circuit:
    ...
    Raises:
        `~OperationNotSupportedError` When the circuit contains an unsupported instruction.
    """
    instructions = []
    measured_qubits = []

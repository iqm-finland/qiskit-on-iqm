...
class IQMBackendBase(BackendV2, ABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        qb_to_idx = {qb: idx for idx, qb in enumerate(architecture.components)}
        operations = {gate_name: gate_info.loci for gate_name, gate_info in architecture.gates.items()}
        target = Target()
        ...
        target.add_instruction(
            IGate(),
            {(qb_to_idx[qb],): None for qb in architecture.components},
        )
